...
for f in diff_file_list: mDiffExec.uses(f, link=Link.OUTPUT, register=True, transfer=True)
for f in new_diff_file_list: mDiffExec.uses(f, link=Link.OUTPUT, register=True, transfer=True)
...
mFitExec.uses("fits.tbl", link=Link.OUTPUT, transfer=True, register=True)
dax.addJob(mFitExec)
...
# for f in new_file_names: mBgExec.uses(f, link=Link.OUTPUT, transfer=True, register=False)
# for f in new_file_names_area: mBgExec.uses(f, link=Link.OUTPUT, transfer=True, register=False)
# dax.addJob(mBgExec)
...
mImgtbl.uses("updated_images.tbl", link=Link.OUTPUT, transfer=True, register=True)
dax.addJob(mImgtbl)
...
mAdd.uses("m101_mosaic.fits", link=Link.OUTPUT, transfer=True, register=True)
dax.addJob(mAdd)
